...
test('fit current session includes breadcrumbs and manual markers',async()=>{const breadcrumbs=[{latitude:59.9,longitude:10.7,recordingSegment:0},{latitude:59.91,longitude:10.71,recordingSegment:0}],markers=[{latitude:59.92,longitude:10.72,type:'note'}];const{context,fitted}=createHarness({breadcrumbs,markers});await flushAsync();assert.equal(run(context,'fitCurrentSession()'),true);assert.equal(fitted.length,1);assert.deepEqual(fitted[0].map(point=>[point.x,point.y]),[[1070,-5990],[1071,-5991],[1072,-5992]]);});
