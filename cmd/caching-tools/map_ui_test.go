...
func TestLocalMapAssetsAreServed(t *testing.T) {
	...
	for _, want := range []string{`id="local-map"`,`id="map-refresh"`,`id="map-zoom-in"`,`id="map-zoom-out"`,`id="map-fit-all"`,`id="map-fit-selection"`,`id="map-selection"`,`src="/map-link.js"`,`src="/map.js"`,`src="/map-overlays.js"`,`Caching Tools M1.40`} { if !strings.Contains(page.Body.String(), want) { t.Fatalf("missing %q from page", want) } }
...
func TestMapOverlayUIAsset(t *testing.T) {
	...
	for _, want := range []string{"Waypoint labels","Waypoint radius","Position / target line","map-overlay-layer","map-overlay-radius","map-overlay-position","map-overlay-guidance","map-overlay-arrival","caching-tools:map-rendered","caching-tools:map-select","/api/waypoints","field-navigation-form","arrival-radius","refreshMapOverlays"} {
