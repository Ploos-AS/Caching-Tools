...
func TestMysteryWorkspaceUIAssets(t *testing.T) {
	...
	for _,want:=range []string{"/api/mystery-workspaces","parseWorkspaceVariables","final_waypoint_id","loadMysteryWorkspaces","Use in solver","caching-tools:puzzle-result","addPuzzleIntermediate","setPuzzleVariable","linkFinalWaypointToActiveWorkspace","Caching Tools M1.17"} { if !strings.Contains(asset.Body.String(),want){t.Fatalf("missing %q",want)} }
