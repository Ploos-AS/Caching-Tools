...
func TestMysteryWorkspaceUIAssets(t *testing.T) {
	...
	for _,want:=range []string{"/api/mystery-workspaces","parseWorkspaceVariables","final_waypoint_id","loadMysteryWorkspaces","Use in solver","caching-tools:puzzle-result","addPuzzleIntermediate","setPuzzleVariable","linkFinalWaypointToActiveWorkspace","Export JSON","/export","/api/mystery-workspaces/import","mystery-workspace-import-file","importWorkspaceFile","Caching Tools M1.19","/path-editor.js"} { if !strings.Contains(asset.Body.String(),want){t.Fatalf("missing %q",want)} }
