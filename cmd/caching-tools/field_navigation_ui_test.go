...
func TestFieldNavigationUIAssets(t *testing.T) {
	...
	for _,want:=range []string{"Field navigation","/api/navigation/field","/api/waypoints","/api/paths","cross_track_m","progress_percent","remaining_m","next_point_distance_m","forward_bearing_deg","off-route-threshold","arrival-radius","guidance.status","off_route_threshold_m","arrival_radius_m","navigator.geolocation","caching-tools:map-select","Caching Tools M1.24"} { if !strings.Contains(asset.Body.String(),want){t.Fatalf("missing %q",want)} }
}
