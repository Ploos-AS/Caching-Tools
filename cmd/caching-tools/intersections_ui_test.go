...
func TestIntersectionWaypointSaveUIAssets(t *testing.T) {
	...
	for _, want := range []string{`id="intersection-bearing-result"`,`id="intersection-distance-result"`,`id="intersection-circle-result"`,`Caching Tools M1.21`,"saved directly as a local waypoint"} { if !strings.Contains(page.Body.String(), want) { t.Fatalf("missing %q from page", want) } }
