...
func TestIntersectionWaypointSaveUIAssets(t *testing.T) {
	...
	h.ServeHTTP(asset, httptest.NewRequest(http.MethodGet, "/intersections.js", nil))
	if asset.Code != http.StatusOK {
		t.Fatalf("intersections.js status=%d", asset.Code)
	}
	for _, want := range []string{
		"Save solution", "/api/waypoints", "type: 'intersection'",
		"loadWaypoints", "refreshLocalMap", "intersection-save-status",
	} {
		if !strings.Contains(asset.Body.String(), want) {
			t.Fatalf("missing %q from intersections.js", want)
		}
	}
}
